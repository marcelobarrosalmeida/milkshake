...
SET OPTS=--verbose  --version="%1" --appname="%APPNAME%" --icon="%ICON%" ^
    --extrasdir=extras --heapsize=4k,5M --caps=%CAPBLS%

echo "Populating temp dir"
if exist "%TMPDIR%" rmdir /s /q "%TMPDIR%"
copy  %SRCDIR%\about.py  %TMPDIR%\extras\data\python\milkshakedir
copy  %SRCDIR%\milkshake.py  %TMPDIR%\extras\data\python\milkshakedir
copy  %SRCDIR%\windows.py  %TMPDIR%\extras\data\python\milkshakedir
